...
public class I02_PokemonFuego extends I00_Pokemon {

	// contructor que hereda de la clase Planeta 

	public I02_PokemonFuego(String nombreEquipo, int tipoPlaneta) {

		super(nombreEquipo, 200, 50, tipoPlaneta);

	}

	public I02_PokemonFuego (String nombreEquipo, int tipoPlaneta, int vidas, int misilesRonda) {
	...
	public int combate (ArrayList<I00_Pokemon> pokemon , int i , int numMisiles , boolean accion, int turno) {
		...
		if (!accion) {
			...
			if (pokemon.get(i).getTipoPlaneta()==3) {
				pokemon.get(i).setVidas(pokemon.get(i).getVidas()-numMisiles/2);

			}else if (pokemon.get(i).getTipoPlaneta()==4) {
				pokemon.get(i).setVidas(pokemon.get(i).getVidas()-numMisiles*2);
