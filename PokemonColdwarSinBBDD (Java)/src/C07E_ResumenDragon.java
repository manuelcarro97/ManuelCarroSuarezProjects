...
public class C07E_ResumenDragon extends JPanel implements ActionListener{

	//Variables de ResumenDragon
	...
	private JLabel jlDragonite;

	//Variables estaticas
	static Clip seSelect;


	public C07E_ResumenDragon() {
		...
		//Color de letra
		Color colorLetra=new Color(77, 77, 140);

		//		animacion y grito del pokemon
		jlDragonite = new JLabel();
		jlDragonite.setBounds(107, 160, 256, 288);
		jlDragonite.setIcon(new ImageIcon("src/resources/pokegifs/dragonite.gif"));			
		add(jlDragonite);
		grito();

		//		resetear el gif para que vuelva a reproducirse desde el principio
		ImageIcon icon = (ImageIcon) jlDragonite.getIcon();
		Image img = icon.getImage();
		ImageIcon newIcon = new ImageIcon(img);
		img.flush();
		jlDragonite.setIcon(newIcon);
		...
		jlDescripcion = new JTextArea("\t\tDATOS"
				...
				+ "\nPP: 10 (+10 cada ronda)\n"
				+ "\nVENTAJA: "
				+ "\nTiene más PS que el resto de tipos."
				+ "\ncada PP de su rival.\n"
				+ "\nDESVENTAJA:"
				+ "\nSólo cuenta con 10 PP al principio pero"
				+ "\nsuma 10 más por cada ronda.\n"
				+ "\nDESCRIPCIÓN:"
				+ "\nDicen que viven en una isla en algún lugar"
				+ "\ndel océano que solo ellos habitan.");
	...
	public void grito() {
		try {
			AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(new File("src/resources/music/SE/dragonite.wav").getAbsoluteFile());
			seSelect = AudioSystem.getClip();
			seSelect.open(audioInputStream);
			seSelect.loop(0);
		} catch(UnsupportedAudioFileException | IOException | LineUnavailableException ex) {
