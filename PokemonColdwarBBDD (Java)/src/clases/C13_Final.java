...
public class C13_Final extends JPanel implements ActionListener {
	...
	public C13_Final() {
		...
		if (C01_Principal.pokemon.size()==1) {
			...
			jlFondo.setIcon(new ImageIcon("src/resources/fondos/final_g.gif"));
			add(jlFondo);
		}else {
			jlFondo = new JLabel();
			jlFondo.setBounds(0,0,1024,768);
			jlFondo.setIcon(new ImageIcon("src/resources/fondos/final_e.gif"));
			add(jlFondo);
		}

		//pausa para reproducir el gif
		Timer clearTimer = new Timer(29350, this);
		clearTimer.start();
