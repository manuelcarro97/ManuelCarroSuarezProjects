...
public class C07D_ResumenPlanta extends JPanel implements ActionListener{

	//Variables de ResumenPlanta
	private JButton jbStart;
	private JButton jbAdd;
	private JLabel jlFondoPlanta;
	...
	private JLabel jlBulbasaur;

	//Variables estaticas
	static Clip seSelect;

	public C07D_ResumenPlanta() {
		...
		//Color de letra
		Color colorLetra=new Color(24, 152, 104);

		//		animacion y grito del pokemon
		jlBulbasaur = new JLabel();
		jlBulbasaur.setBounds(135, 265, 196, 180);
		jlBulbasaur.setIcon(new ImageIcon("src/resources/pokegifs/bulbasaur.gif"));			
		add(jlBulbasaur);
		grito();

		//		resetear el gif para que vuelva a reproducirse desde el principio
		ImageIcon icon = (ImageIcon) jlBulbasaur.getIcon();
		Image img = icon.getImage();
		ImageIcon newIcon = new ImageIcon(img);
		img.flush();
		jlBulbasaur.setIcon(newIcon);

		//Texto tipo Pokemon
		jlTipo = new JLabel("Tipo PLANTA");
		...
		jlDescripcion = new JTextArea("\t\tDATOS"
				...
				+ "\nPP: 50\n"
				+ "\nVENTAJA:"
				+ "\nHace el doble de daño al tipo AGUA.\n"
				+ "\nDESVENTAJA:"
				+ "\nRecibe el doble de daño del tipo FUEGO.\n"
				+ "\nDESCRIPCIÓN:"
				+ "\nEste Pokémon nace con una semilla en su"
				+ "\nlomo, la cual brota con el paso del tiempo.");
		...
		//Fondo
		jlFondoPlanta = new JLabel();
		jlFondoPlanta.setBounds(0,0,1024,768);
		jlFondoPlanta.setIcon(new ImageIcon("src/resources/fondos/FondosPokedex/tipoplanta.png"));
		add(jlFondoPlanta);
	...
	public void grito() {
		try {
			AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(new File("src/resources/music/SE/bulbasaur.wav").getAbsoluteFile());
			seSelect = AudioSystem.getClip();
			seSelect.open(audioInputStream);
			seSelect.loop(0);
		} catch(UnsupportedAudioFileException | IOException | LineUnavailableException ex) {
