...
public class C07A_ResumenNormal extends JPanel implements ActionListener{

	//Variables de ResumenNormal
	private JButton jbStart;
	private JButton jbAdd;
	private JLabel jlFondoNormal;
	...
	public C07A_ResumenNormal() {
		...
		//Color de letra
		Color colorLetra=new Color(213, 156, 74);

		//		animacion y grito del pokemon
		jlEevee = new JLabel();
		jlEevee.setBounds(160, 244, 152, 204);
		jlEevee.setIcon(new ImageIcon("src/resources/pokegifs/eevee.gif"));			
		add(jlEevee);
		grito();

		//		resetear el gif para que vuelva a reproducirse desde el principio
		ImageIcon icon = (ImageIcon) jlEevee.getIcon();
		Image img = icon.getImage();
		ImageIcon newIcon = new ImageIcon(img);
		img.flush();
		jlEevee.setIcon(newIcon);

		//Texto tipo Pokemon
		jlTipo = new JLabel("Tipo NORMAL");
		...
		jlDescripcion = new JTextArea("\t\tDATOS"
				...
				+ "\nPP: 50\n"
				+ "\nDESCRIPCIÓN:"
				+ "\nEs capaz de alterar la composición de su"
				+ "\ncuerpo para confundirse con el entorno.");
		...
		//Fondo de tipo normal
		jlFondoNormal = new JLabel();
		jlFondoNormal.setBounds(0,0,1024,768);
		jlFondoNormal.setIcon(new ImageIcon("src/resources/fondos/FondosPokedex/tiponormal.png"));
		add(jlFondoNormal);
	}
	...
	public void grito() {
		try {
			AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(new File("src/resources/music/SE/eevee.wav").getAbsoluteFile());
			seSelect = AudioSystem.getClip();
			seSelect.open(audioInputStream);
			seSelect.loop(0);
		} catch(UnsupportedAudioFileException | IOException | LineUnavailableException ex) {
