...
public class C07C_ResumenAgua extends JPanel implements ActionListener{

	//Variables de ResumenAgua
	private JButton jbStart;
	private JButton jbAdd;
	private JLabel jlFondoAgua;
	...
	private JLabel jlSquirtle;

	//Variables estaticas
	static Clip seSelect;


	public C07C_ResumenAgua() {
		...
		//Color de letra
		Color colorLetra=new Color(40, 136, 240);

		//		animacion y grito del pokemon
		jlSquirtle = new JLabel();
		jlSquirtle.setBounds(112, 292, 220, 160);
		jlSquirtle.setIcon(new ImageIcon("src/resources/pokegifs/squirtle.gif"));			
		add(jlSquirtle);
		grito();

		//		resetear el gif para que vuelva a reproducirse desde el principio
		ImageIcon icon = (ImageIcon) jlSquirtle.getIcon();
		Image img = icon.getImage();
		ImageIcon newIcon = new ImageIcon(img);
		img.flush();
		jlSquirtle.setIcon(newIcon);

		//Texto tipo Pokemon
		jlTipo = new JLabel("Tipo AGUA");
		...
		jlDescripcion = new JTextArea("\t\tDATOS"
				...
				+ "\nPP: 50\n"
				+ "\nVENTAJA:"
				+ "\nHace el doble de daño al tipo FUEGO.\n"
				+ "\nDESVENTAJA:"
				+ "\nRecibe el doble de daño del tipo PLANTA.\n"
				+ "\nDESCRIPCIÓN:"
				+ "\nCuando retrae su largo cuello en el "
				+ "\ncaparazón, dispara agua a una presión"
				+ "\nincreíble.");
		...
		//Fondo
		jlFondoAgua = new JLabel();
		jlFondoAgua.setBounds(0,0,1024,768);
		jlFondoAgua.setIcon(new ImageIcon("src/resources/fondos/FondosPokedex/tipoagua.png"));
		add(jlFondoAgua);
	...
	public void grito() {
		try {
			AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(new File("src/resources/music/SE/squirtle.wav").getAbsoluteFile());
			seSelect = AudioSystem.getClip();
			seSelect.open(audioInputStream);
			seSelect.loop(0);
		} catch(UnsupportedAudioFileException | IOException | LineUnavailableException ex) {
